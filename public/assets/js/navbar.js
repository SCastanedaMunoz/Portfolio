$(document).ready(function () {
  ...
  const navBarContents = [
    ...
    {
      type: "icon",
      icon_light: "./assets/images/icons/GitHub-Light.png",
      icon_dark: "./assets/images/icons/GitHub-Dark.png",
  ...
  var sideNavTarget = $(
    `<a href="#" data-target="mobile-sidenav" class="sidenav-trigger" </a>`
  );
  ...
  function fillNavElement(toAppend, location) {
    ...
    navBarContents.forEach((value) => {
      ...
      switch (value.type) {
        case "page":
          var link = $(
            `<a href="${value.link}" target="${value.target}">${value.text}</a>`
          );
          if (value.link == currentPage) $(contentItem).addClass("active");
        ...
        case "icon":
          var icon =
            value.text == "GitHub"
              ...
              : value.icon;
          var link = $(
            `<a href="${value.link}" target="_blank"><img src="${icon}" alt="${value.text} Social" class="social-icon-${location}"></a>`
          );
          $(contentItem).append(link);
          break;
        case "dropdown":
          var link = $(
            `<a class="dropdown-trigger" href="#!" data-target="${value.link}-${location}"><i class="material-icons right">arrow_drop_down</i>${value.text}</a>`
          );

          var dropDownStructure = $(
            `<ul id="${value.link}-${location}" class="dropdown-content"></ul>`
          );
          value.drop_content.forEach((element) => {
            var liElement = $(
              `<li><a href="#!" class="dropdown-link" data-summary="${element.summary}" data-ss="${element.path_screenshot}" data-deploy="${element.link_deploy}" data-github="${element.link_github}" data-title="${element.title}">${element.title}</a></li>`
            );
